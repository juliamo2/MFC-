...
namespace easypr
{
	...
	private:
		//! plate mat
		Mat m_plateMat;				// 计算得到车牌图形块

		//! plate rect
		RotatedRect m_platePos;		// 车牌区域在原图像中的位置区域

		//! plate license
		String m_plateStr;			// 识别出来的车牌号

		//! plate locate type
		LocateType m_locateType;	// 车牌区域定位方式

		//! plate color type
		Color m_plateColor;			// 车牌的颜色

		//! scale ratio
		float m_scale;

		//! plate likely
		double m_score;				// 检测为该车牌号码的概率
