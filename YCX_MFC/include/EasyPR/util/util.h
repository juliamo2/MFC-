...
namespace easypr
{
	...
	public:
		...
		// 在原始图像上，画出指定矩形区域
		static bool drawRect(cv::Mat& src, cv::Rect2f rect, cv::Scalar color);
		static bool drawRect(cv::Mat& src, cv::RotatedRect rotatedRect, cv::Scalar color);
		static bool drawRect(cv::Mat& src, std::vector<cv::Rect2f> rects, cv::Scalar color);
		static bool drawRect(cv::Mat& src, std::vector<cv::RotatedRect> rotatedRects, cv::Scalar color);
